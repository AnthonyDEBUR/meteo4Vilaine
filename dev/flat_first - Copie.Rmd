...
#'  crs = 4326
#')
#' df <- precipitations_par_zone(triangle_sf, date = "2022-01-10", nb_jours = 5, con = con)
#' 
#' # Coordonnées approximatives du centre de Theix-Noyalo
...
#' df2 <- precipitations_par_zone(carre, date = "2022-01-10", nb_jours = 5, con = con)
#' 
#' }
precipitations_par_zone <- function(sf_objet, date, nb_jours, con) {
  ...
  stations_dans_zone <- sf::st_intersection(pluvio_temp, sf_objet)
    ...
    if (length(dates_manquantes) > 0) {
      for (k in seq_along(dates_manquantes)) {
        date_i<-dates_manquantes[k]
